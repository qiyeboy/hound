	...
	for _, file := range files {
		...
		if err := g.grep2File(filepath.Join(n.Ref.dir, "raw", name), re, int(opt.LinesOfContext),
			func(line []byte, lineno int, before [][]byte, after [][]byte) (bool, error) {
				...
				// log.Printf("line is %v, lineno is %v, before is %v after is %v\n", line, lineno, before, after)
				...
				matchesCollected++
				// log.Printf("matchesCollected is %v\n", matchesCollected)
		...
		if len(matches) > 0 {
			...
			// log.Printf("results is v+%\n", results)

		}
	}
	// log.Printf("filesFound is v+%\n", filesFound)
	// log.Printf("filesOpened is v+%\n", filesOpened)
	// log.Printf("n.Ref.Rev is v+%\n", n.Ref.Rev)
...
func containsString(haystack []string, needle string) bool {
	for i, n := 0, len(haystack); i < n; i++ {
		if haystack[i] == needle {
			// fmt.Printf("exclude files :%v, as %v\n", needle, haystack[i])
			return true
		}
	}
	return false
...
func indexAllFiles(opt *IndexOptions, dst, src string) error {
	// fmt.Printf("dst is %v\n,src is %v\n", dst, src)
	// create a new indexDir-tri directory
...
func Build(opt *IndexOptions, dst, src, url, rev string) (*IndexRef, error) {
	...
	// create a index-raw dir
	if err := os.Mkdir(filepath.Join(dst, "raw"), os.ModePerm); err != nil {
		return nil, err
	}

	// here we index every file in index-dir, filter logic in it
