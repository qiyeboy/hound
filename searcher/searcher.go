...
func (s *Searcher) Search(pat string, opt *index.SearchOptions) (*index.SearchResponse, error) {
	...
	log.Printf("%v", runtime.FuncForPC(pc).Name())
	s.lck.RLock()
	defer s.lck.RUnlock()
...
func buildAndOpenIndex(
	...
	return index.Open(idxDir) // index exsit, return directly, second logic
}
...
func updateAndReindex(
	...
	newRev := repo.Url
	var err error
	isRepo := strings.HasPrefix(repo.Url, "http")
	if isRepo == true {
...
func newSearcher(
	...
	// for local
	vcsDir := repo.Url                             // vcsDir source path
	rev := repo.Url                                //rev;
	wd, err := vcs.New(repo.Vcs, repo.VcsConfig()) // wd :
